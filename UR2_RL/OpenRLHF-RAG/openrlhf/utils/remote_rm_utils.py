...
def remote_rm_fn(api_url, queries, epoch, score_key="rewards"):
    ...
    # print("sht-debug-"*10)
    scores, format_rewards, sources, is_retrieves = request_api_wrapper(api_url, {"query": queries, "epoch": epoch}, score_key)
#    print("hi i am here")
    return torch.tensor(scores), format_rewards, sources, is_retrieves


@ray.remote
def remote_rm_fn_ray(api_url, queries, epoch, score_key="rewards"):
    return remote_rm_fn(api_url, queries, epoch, score_key)
